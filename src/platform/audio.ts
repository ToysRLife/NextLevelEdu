import type { AudioService } from "@sdk/types";
import { read, write } from "./storage";

type Sound = Parameters<AudioService["play"]>[0];

// Tiny synth so we ship satisfying feedback with zero audio assets.
const TONES: Record<Sound, { freq: number; dur: number; type: OscillatorType }[]> = {
  success: [
...
let muted = read<boolean>("audio:muted", false);
