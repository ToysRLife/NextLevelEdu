...
class Fossils implements GameInstance {
  ...
  private dig(p: Point): void {
    if (this.ended) return;
    if (p.y < DIG_TOP) return;
    const cc = (p.x - 0) / CW;
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Dig complete! Fossils are clues to life long ago. Rock piles up in layers over time, so the deeper you dig, the older the fossils you find.",
      stars,
      resources: { Rock: 40 },
    });
  }
...
export const fossilsGame: GameModule = {
  meta: {
    id: "fossils",
    conceptId: "ess-19",
    title: "Fossil Dig",
