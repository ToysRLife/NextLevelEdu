...
interface Material {
  ...
  label: string;
  emoji: string;
  soak: number; // fraction of puddle removed per dab
}
...
class Absorbency implements GameInstance {
  ...
  private active = "sponge";
  private volume = 1; // 0..1
  private wasted = 0;
  private splashes: { x: number; y: number; life: number; ok: boolean }[] = [];
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "All mopped up! Absorbent materials like sponges and towels soak water into tiny spaces inside them. Smooth, waterproof ones let it run right off.",
      stars,
  ...
  private renderLoop(): void {
    const draw = () => {
      this.anim += 0.05;
      for (const s of this.splashes) s.life -= 0.04;
...
export const absorbencyGame: GameModule = {
  meta: {
    id: "absorbency",
    conceptId: "chem-20",
    title: "Soak It Up",
