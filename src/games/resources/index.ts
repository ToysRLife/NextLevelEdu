...
const RESOURCES: Resource[] = [
  ...
  { name: "Fresh water", emoji: "💧", use: "drink", why: "We use fresh water for drinking and washing." },
  { name: "Sunlight", emoji: "☀️", use: "solar", why: "Solar panels turn sunlight into electricity." },
  { name: "Oil", emoji: "🛢️", use: "fuel", why: "Oil is refined into fuel for cars and planes." },
  { name: "Rock & minerals", emoji: "🪨", use: "build", why: "Rock and minerals build our homes and make metal." },
];
...
class Resources implements GameInstance {
  ...
  private current(): Resource { return this.order[this.idx]; }
  ...
  private buildPanel(): void {
    ...
    this.ctx.panel.append(
      el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🌍 Resource"), el("span", {}, this.current().name)),
  ...
  private choose(key: string): void {
    ...
    if (key === r.use) { this.flash = 1; this.ctx.services.audio.play("tick"); this.coachEl.textContent = `✅ ${r.why}`; }
    else { this.flash = -1; this.mistakes += 1; this.ctx.services.audio.play("fail"); this.coachEl.textContent = `❌ ${r.why}`; }
    this.idx += 1;
    if (this.idx >= this.order.length) { this.progressEl.textContent = `${RESOURCES.length} / ${RESOURCES.length}`; this.win(); }
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message: "Resourceful! Natural resources from the Earth — water, trees, sunlight, oil, and minerals — give us nearly everything we use.",
      stars, resources: { Minerals: 40 },
    });
  }
  ...
  private render(): void {
    ...
    if (this.ended) { c.fillStyle = "#9a3412"; c.font = "bold 30px Nunito, sans-serif"; c.textAlign = "center"; c.fillText("All matched! 🌍", W / 2, H / 2); return; }
    ...
    c.font = "120px serif"; c.textAlign = "center"; c.fillText(r.emoji, cx, cy + 20);
    c.fillStyle = "#9a3412"; c.font = "bold 26px Nunito, sans-serif"; c.fillText(r.name, cx, cy + 100);
    c.font = "bold 20px Nunito, sans-serif"; c.fillText("What do we use it for? 🌍", W / 2, 60);
  }
  ...
  pause(): void {}
  resume(): void {}
  reset(): void { this.ended = false; this.idx = 0; this.mistakes = 0; this.flash = 0; this.order = [...RESOURCES].sort(() => Math.random() - 0.5); this.ctx.services.hints.reset(); this.buildPanel(); }
  destroy(): void { cancelAnimationFrame(this.raf); }
}

export const resourcesGame: GameModule = {
  meta: {
    id: "resources", conceptId: "ess-20", title: "Earth's Gifts", stream: "earth-space", gradeBand: "4",
    emoji: "🌍", blurb: "Match each natural resource to what we use it for.",
    mission: "Match every natural resource to its main use.", estMinutes: 2,
  },
