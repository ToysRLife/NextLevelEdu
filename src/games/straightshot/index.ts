...
class StraightShot implements GameInstance {
  ...
  private buildPanel(): void {
    const slider = el("input", {
      type: "range", min: "100", max: "500", value: String(this.holeY),
      "aria-label": "Hole height", style: { accentColor: "var(--accent-yellow)" },
      oninput: (e: Event) => { this.holeY = Number((e.target as HTMLInputElement).value); this.updateReadout(); },
    });
    ...
    this.ctx.panel.append(
      el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Goal"), el("span", {}, `Reach the eye ${NEED}×`)),
      el("div", { class: "control-label", style: { marginTop: "8px" } }, "🕳️ Move the hole up/down"),
      slider,
  ...
  private updateReadout(): void {
    ...
    this.alignEl.textContent = Math.abs(d) < this.tol ? "lined up 🎯" : d < 0 ? "hole too high" : "hole too low";
  }
  ...
  private renderLoop(): void {
    const draw = () => {
      this.anim += 0.05;
      if (!this.ended) {
        if (Math.abs(this.holeY - this.requiredHoleY()) < this.tol) { this.hold += 1; if (this.hold > 36) this.hit(); }
        else this.hold = 0;
      }
      this.render();
  ...
  private hit(): void {
    ...
    else { this.placeTarget(); this.coachEl.textContent = "👁️ The light got through! Line up the next one."; this.updateReadout(); }
  }
  ...
  private render(): void {
    ...
    // beam
    c.strokeStyle = "#fde047"; c.lineWidth = 5;
    c.beginPath(); c.moveTo(LAMP_X, LAMP_Y);
    if (aligned) { c.lineTo(WALL_X, this.holeY); c.lineTo(EYE_X, this.targetY); }
    else {
    ...
    // lamp
    c.fillStyle = "#facc15"; c.beginPath(); c.arc(LAMP_X, LAMP_Y, 18, 0, Math.PI * 2); c.fill();
    c.font = "22px serif"; c.textAlign = "center"; c.fillText("🔦", LAMP_X, LAMP_Y + 7);
    // eye
    c.font = "40px serif"; c.fillText(aligned ? "😃" : "👁️", EYE_X, this.targetY + 12);
    if (aligned) { c.fillStyle = "rgba(253,224,71,0.3)"; c.beginPath(); c.arc(EYE_X, this.targetY, 30 + Math.sin(this.anim * 3) * 4, 0, Math.PI * 2); c.fill(); }

    c.fillStyle = "rgba(255,255,255,0.9)"; c.font = "bold 18px Nunito, sans-serif"; c.textAlign = "center";
    c.fillText("Line the hole up so the light reaches the eye 👁️", W / 2, 40);
    if (this.hold > 0 && !this.ended) { c.fillStyle = "#86efac"; c.font = "bold 14px Nunito, sans-serif"; c.fillText("hold it…", WALL_X, this.holeY - 30); }
  }

  start(): void {}
  pause(): void {}
  resume(): void {}
  reset(): void { this.ended = false; this.holeY = 300; this.caught = 0; this.hold = 0; this.placeTarget(); this.ctx.services.hints.reset(); this.buildPanel(); }
  destroy(): void { cancelAnimationFrame(this.raf); }
}

export const straightShotGame: GameModule = {
  meta: {
    id: "straightshot", conceptId: "phys-12", title: "Straight Shot", stream: "physics", gradeBand: "4",
    emoji: "🔦", blurb: "Line up the hole so the straight beam of light reaches the eye.",
    mission: "Use the fact that light travels straight to get the beam through to the eye.", estMinutes: 2,
  },
