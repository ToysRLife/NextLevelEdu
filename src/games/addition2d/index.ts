...
class Addition2D implements GameInstance {
  ...
  private readonly newProblemBtn: HTMLButtonElement;
  ...
  private readonly numpad: HTMLElement;
  private readonly learnPanel: HTMLElement;
  private readonly aSlider: HTMLInputElement;
  private readonly bSlider: HTMLInputElement;
  private readonly aLabel: HTMLElement;
  private readonly bLabel: HTMLElement;

  private mode: "play" | "learn" = "play";
  private a = 0;
  private b = 0;
  private selected = -1;
  private score = 0;
  ...
  constructor(private readonly ctx: GameContext) {
    ...
    this.demoBtn = el(
      "button",
      { class: "btn secondary", onclick: () => this.showDemonstration() },
      "Show"
    );
    this.newProblemBtn = el(
      "button",
      { class: "btn", onclick: () => this.newProblem() },
      "New Problem"
    );
    ...
    this.learnPanel = el(
      "div",
      { style: { display: "none", flexDirection: "column", gap: "12px", marginTop: "12px" } },
      el("div", { style: { fontWeight: "800" } }, "Try a demonstration"),
      el(
        "div",
  ...
  private buildPanel(): void {
    ...
    this.modeButtons = el(
      ...
      el("button", { class: "btn secondary", onclick: () => this.setMode("learn") }, "Learn Mode"),
      el("button", { class: "btn", onclick: () => this.setMode("play") }, "Play Mode"),
      this.newProblemBtn,
      el("div", { style: { marginLeft: "auto", fontWeight: "700" } }, "Score: ", this.scoreEl)
    );

    const problemRow = el(
      "div",
      { class: "metric", style: { justifyContent: "space-between" } },
      el("span", {}, "Problem"),
      this.problemEl
    );
  ...
  private setMode(mode: "play" | "learn"): void {
    this.mode = mode;
    if (mode === "learn") {
  ...
  private showDemonstration(): void {
    this.a = Number(this.aSlider.value);
    this.b = Number(this.bSlider.value);
...
export const addition2dGame: GameModule = {
  meta: {
    id: "addition2d",
    conceptId: "phys-02",
    title: "Add It Up",
