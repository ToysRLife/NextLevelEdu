...
type Fate = "Red Dwarf" | "White Dwarf" | "Neutron Star" | "Black Hole";
const fateFor = (mass: number): Fate =>
  mass < 0.5 ? "Red Dwarf" : mass < 8 ? "White Dwarf" : mass < 20 ? "Neutron Star" : "Black Hole";
...
const ROUNDS: Fate[] = ["White Dwarf", "Black Hole", "Neutron Star"];

class StarLife implements GameInstance {
  ...
  private fateRead!: { el: HTMLElement; set(v: string): void };
  ...
  private evolve(): void {
    if (this.ended || this.evolving) return;
    this.evolving = true;
    this.prog = 0;
    this.acc = 0;
  ...
  private massForTargetHint(): string {
    const t = this.target();
    return t === "Red Dwarf"
  ...
  private finish(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Stellar! A star's mass decides its destiny: light stars fade into white dwarfs, while heavy ones explode as supernovae and leave neutron stars or black holes.",
      stars,
  ...
  private render(): void {
    ...
    for (let i = 0; i < 60; i++) c.fillRect((i * 137) % W, (i * 89) % H, 2, 2);
...
export const starLifeGame: GameModule = {
  meta: {
    id: "starlife",
    conceptId: "ess-27",
