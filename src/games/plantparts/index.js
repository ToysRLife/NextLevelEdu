...
class PlantParts {
    ...
    buildPanel() {
        ...
        const chips = el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...choices.map((p) => el("button", { class: "chip", onclick: () => this.choose(p.key) }, p.jobShort)));
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Part"), el("span", {}, this.current().name)), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Pick this part's job"), chips, el("div", { class: "metric" }, el("span", {}, "🌱 Part"), this.progressEl), this.coachEl);
    }
    choose(key) {
        if (this.ended)
            return;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Plant expert! Roots drink water and anchor the plant, the stem holds it up and carries water, leaves make food from sunlight, and flowers make seeds.",
            stars,
            resources: { Seeds: 40 },
        });
    }
