...
class Hazards implements GameInstance {
  ...
  private buildPanel(): void {
    const chips = el(
      ...
      ...ACTIONS.map((a) =>
        el("button", { class: "chip", onclick: () => this.choose(a.key) }, `${a.emoji} ${a.label}`)
      )
    );
    this.progressEl = el(
      "span",
      { style: { color: "var(--accent-orange)" } },
      `${this.idx + 1} / ${HAZARDS.length}`
    );
    this.coachEl = el("div", {
      class: "hint-panel",
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Safety star! Knowing what to do in earthquakes, floods, fires, and storms keeps you safe when nature gets wild.",
      stars,
      resources: { Rock: 40 },
    });
  }
