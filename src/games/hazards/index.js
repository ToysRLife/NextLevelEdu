...
const HAZARDS = [
    {
        ...
        emoji: "🌍",
        action: "cover",
        why: "In an earthquake, drop, cover, and hold on under something sturdy.",
    },
    {
        name: "Flood",
        emoji: "🌊",
        action: "high",
        why: "In a flood, move to higher ground away from the rising water.",
    },
    {
        name: "Thunderstorm",
        emoji: "⛈️",
        action: "inside",
        why: "In a thunderstorm, shelter inside away from the lightning.",
    },
    {
        name: "Wildfire",
        emoji: "🔥",
        action: "away",
        why: "In a wildfire, move far away from the flames and smoke.",
    },
    {
        name: "Tornado",
        emoji: "🌪️",
        action: "inside",
        why: "In a tornado, shelter inside, in a basement or inner room.",
    },
];
class Hazards {
    constructor(ctx) {
    ...
    buildPanel() {
        const chips = el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...ACTIONS.map((a) => el("button", { class: "chip", onclick: () => this.choose(a.key) }, `${a.emoji} ${a.label}`)));
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "⚠️ Hazard"), el("span", {}, this.current().name)), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Choose the safe action"), chips, el("div", { class: "metric" }, el("span", {}, "🚨 Hazard"), this.progressEl), this.coachEl);
    }
    choose(key) {
        if (this.ended)
            return;
        ...
        if (key === h.action) {
            ...
            this.coachEl.textContent = `✅ ${h.why}`;
        }
        else {
            this.flash = -1;
            this.mistakes += 1;
        ...
        if (this.idx >= this.order.length) {
            this.progressEl.textContent = `${HAZARDS.length} / ${HAZARDS.length}`;
            this.win();
        }
        else
            this.progressEl.textContent = `${this.idx + 1} / ${HAZARDS.length}`;
    }
    win() {
        this.ended = true;
    ...
    render() {
        ...
        if (this.ended) {
            ...
            c.textAlign = "center";
            c.fillText("Stay safe out there! 🚨", W / 2, H / 2);
            return;
        }
        if (this.flash > 0) {
        ...
        c.font = "120px serif";
        c.textAlign = "center";
        c.fillText(h.emoji, cx, cy + 20);
        c.fillStyle = "#9a3412";
        c.font = "bold 26px Nunito, sans-serif";
        c.fillText(h.name, cx, cy + 100);
        c.font = "bold 20px Nunito, sans-serif";
        c.fillText("Stay safe — what should you do? ⚠️", W / 2, 60);
    }
