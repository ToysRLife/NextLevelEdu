...
class SoilLayers implements GameInstance {
  ...
  private pick(key: string): void {
    ...
    if (key === correct.key) {
      ...
      if (this.placed >= LAYERS.length) this.win();
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Soil sorted! Soil is built in layers — rotting leaves on top, then rich topsoil, then mineral subsoil, and solid bedrock at the bottom.",
      stars,
...
export const soilLayersGame: GameModule = {
  meta: {
    id: "soillayers",
    conceptId: "ess-15",
    title: "Soil Layers",
