...
class AnimalGroups {
    ...
    buildPanel() {
        const chips = el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...GROUPS.map((g) => el("button", { class: "chip", onclick: () => this.choose(g.key) }, `${g.emoji} ${g.label}`)));
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Goal"), el("span", {}, "Classify every animal")), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Which group does it belong to?"), chips, el("div", { class: "metric" }, el("span", {}, "🦓 Animal"), this.progressEl), this.coachEl);
    }
    choose(group) {
        if (this.ended)
            return;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Zoologist! Animals are grouped by shared features — fur and milk for mammals, feathers for birds, gills for fish, scales for reptiles, moist skin for amphibians, six legs for insects.",
            stars,
