...
class SolarSystem {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "placed", {
            ...
            writable: true,
            value: 0
        }); // how many planets correctly placed (from Sun out)
    ...
    buildPanel() {
        ...
        const trayChips = el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...this.tray.map((p) => el("button", { class: "chip", onclick: () => this.pick(p.key) }, `${p.chip} ${p.name}`)));
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Goal"), el("span", {}, "Order all 8 planets")), el("div", { class: "control-label", style: { marginTop: "8px" } }, `Place orbit #${Math.min(this.placed + 1, 8)} (from the Sun)`), trayChips, el("div", { class: "metric" }, el("span", {}, "🪐 Placed"), this.statusEl), this.coachEl);
    }
    pick(key) {
        if (this.ended)
            return;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "The solar system is built! Eight planets orbit the Sun — four small rocky worlds close in, then four giant gas and ice planets far out.",
            stars,
