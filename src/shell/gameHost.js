...
import { addJoules, addResources, getLevel, recordPerformance, recordPlayToday, recordRecent, } from "./profile";
...
function makeFeedbackBlock(gameId, title = "How was this game?") {
    ...
    const sendRow = el("div", { class: "wf-send", style: { display: "none" } }, comment, el("button", {
        class: "btn small",
        onclick: async (e) => {
    ...
    const facesRow = el("div", { class: "wf-faces" }, ...faces.map(([r, emoji]) => el("button", {
        class: "wf-face",
        onclick: (e) => {
            chosen = r;
...
export function renderGameHost(root, meta) {
    ...
    const feedbackBtn = el("button", { class: "btn secondary", onclick: () => openFeedback() }, "💬 Feedback");
    const actions = el("div", { class: "host-actions" }, hintBtn, feedbackBtn, restartBtn, backBtn);
    const missionBanner = el("div", { class: "mission-banner" }, el("span", { class: "label" }, "Mission"), el("span", {}, meta.mission));
    ...
    function openFeedback() {
        clearOverlay();
        const card = el("div", { class: "outcome-card" }, el("div", { class: "outcome-emoji" }, "💬"), el("h2", {}, "Tell us how it's going"), makeFeedbackBlock(meta.id, "Is everything working? Let us know!"), el("div", { class: "outcome-actions" }, el("button", { class: "btn secondary", onclick: () => clearOverlay() }, "Close")));
        canvasWrap.append(el("div", { class: "overlay" }, card));
    }
    function showOutcome(kind, detail) {
        clearOverlay();
        ...
        if (success) {
            ...
            addJoules(reward);
            if (detail.resources)
                addResources(meta.stream, detail.resources);
        ...
        if (newBadges.length)
            services.audio.play("reward");
        const rewardPills = detail.resources && Object.keys(detail.resources).length
            ? el("div", { class: "rewards" }, ...Object.entries(detail.resources).map(([name, amount]) => el("div", { class: "reward-pill" }, `+${amount} ${name}`)))
            : null;
        ...
        if (reward) {
            if (reward.complete) {
                ...
                const bars = Object.entries(n.cost).map(([name, amt]) => {
                    ...
                    return el("div", { class: "res-row" }, el("span", { class: "res-name" }, name), el("div", { class: "res-bar" }, el("div", {
                        class: `res-fill ${cur >= amt ? "full" : ""}`,
                        style: { width: `${p}%` },
                    })), el("span", { class: "res-num" }, `${cur}/${amt}`));
                });
                worldBlock = el("div", { class: `win-world ${n.affordable ? "ready" : ""}` }, el("div", { class: "win-world-head" }, `${reward.emoji} ${reward.name}`), el("div", { class: "win-world-next" }, n.affordable ? `Ready to build: ${n.emoji} ${n.name}!` : `Next: ${n.emoji} ${n.name}`), ...bars);
            }
        }
        // 1-card concept recap — what the learner just discovered.
        const recapBlock = success && meta.takeaway
        ...
        const badgeBlock = newBadges.length
            ? el("div", { class: "win-badges" }, el("div", { class: "wb-title" }, newBadges.length > 1 ? "🏅 New badges!" : "🏅 New badge!"), el("div", { class: "wb-row" }, ...newBadges.map((b) => el("div", { class: "wb-badge", title: b.desc }, el("span", { class: "wb-emoji" }, b.emoji), el("span", {}, b.title)))))
            : null;
        const card = el("div", { class: `outcome-card ${success ? "success" : "fail"}` }, el("div", { class: "outcome-emoji" }, success ? "🎉" : "💪"), el("h2", {}, success ? "Mission Complete!" : "Not quite — try again!"), success ? el("div", { class: "stars" }, "⭐".repeat(stars) + "☆".repeat(3 - stars)) : null, el("p", {}, detail.message ?? (success ? "Brilliant work!" : "Every scientist learns by trying again.")), recapBlock, rewardPills, worldBlock, badgeBlock, makeFeedbackBlock(meta.id), el("div", { class: "outcome-actions" }, el("button", {
            class: "btn",
            onclick: () => {
                clearOverlay();
                hintPanel.style.display = "none";
        ...
        }, success ? "Play Again" : "Retry"), success
            ? el("button", {
                class: "btn world",
                onclick: () => {
    ...
    let cancelled = false;
    const loadingOverlay = el("div", { class: "overlay" }, el("div", { class: "outcome-card" }, el("div", { class: "outcome-emoji" }, "⏳"), el("p", {}, "Loading…")));
