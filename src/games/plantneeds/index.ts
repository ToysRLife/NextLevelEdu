...
// Each need has a "just right" band. Too little or too much both hurt — a gentle
// lesson that plants need water, sunlight, and air in the right amounts.
interface Need {
  key: "water" | "sun" | "air";
  label: string;
  emoji: string;
  low: number; // ideal band start (0..100)
  high: number; // ideal band end
  tooLow: string;
  tooHigh: string;
}
...
class PlantNeeds implements GameInstance {
  ...
  private vals: Record<string, number> = { water: 20, sun: 20, air: 20 };
  private growth = 0; // 0..1
  ...
  private tick(dtMs: number): void {
    ...
    let happy = 0;
    let worst: { msg: string } | null = null;
    ...
    if (happy === NEEDS.length) {
      ...
      if (worst) this.coachEl.textContent = `⚠️ ${worst.msg}`;
    }

    this.growthEl.textContent = `${Math.round(this.growth * 100)}%`;

    if (this.growth >= 1) this.finish();
    this.render();
  }
  ...
  private finish(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "It bloomed! 🌸 Plants are living things that need water, sunlight, and air — in just the right amounts — to grow.",
      stars,
      resources: { Water: 40 },
    });
  }
...
export const plantNeedsGame: GameModule = {
  meta: {
    id: "plantneeds",
    conceptId: "bio-03",
