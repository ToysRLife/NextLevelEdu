...
interface Round {
  target: number; // must be ≥ this at the check time
  checkMin: number; // minutes
}
const ROUNDS: Round[] = [
...
const coolingK = (insul: number) => 0.12 / (1 + insul * 0.45);
const tempAt = (insul: number, min: number) => ENV + (T0 - ENV) * Math.exp(-coolingK(insul) * min);

class Cocoa implements GameInstance {
  ...
  private t = 0; // minutes elapsed in the sim
  private points: [number, number][] = [];
  private acc = 0;

  private insulCtl!: SliderHandle;
  private predRead!: { el: HTMLElement; set(v: string): void };
  ...
  private tick(dtMs: number): void {
    ...
    if (this.running) {
      ...
      while (this.running && this.acc >= 16.67 && steps < 30) {
        ...
        if (this.t >= this.round().checkMin) this.evaluate();
        this.acc -= 16.67;
        steps++;
      }
    }
  ...
  private finish(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Toasty! Heat flows from hot to cold, and insulation slows that flow — more insulation means a gentler cooling curve, so the cocoa stays warm longer.",
      stars,
      resources: { Oxygen: 60 },
    });
  }
...
export const cocoaGame: GameModule = {
  meta: {
    id: "cocoa",
    conceptId: "phys-29",
