...
const ITEMS: Item[] = [
  { name: "Fruit salad", emoji: "🥗", mixture: true, why: "You can pick the pieces back out — it's a mixture." },
  { name: "Trail mix", emoji: "🥜", mixture: true, why: "The nuts and raisins keep their own properties — a mixture." },
  { name: "Sand + iron filings", emoji: "🧲", mixture: true, why: "A magnet pulls the iron out — they were just mixed, not joined." },
  { name: "Salt in water", emoji: "🧂", mixture: true, why: "Evaporate the water and the salt comes back — it's a mixture (a solution)." },
  { name: "Cereal + milk", emoji: "🥣", mixture: true, why: "The cereal and milk stay themselves — a mixture." },
  { name: "Baked cake", emoji: "🎂", mixture: false, why: "Baking made a brand-new substance — you can't get the eggs and flour back." },
  { name: "Burnt toast", emoji: "🍞", mixture: false, why: "Burning created new stuff (carbon) — not a mixture you can separate." },
  { name: "Cooked egg", emoji: "🍳", mixture: false, why: "Cooking changed the egg into a new substance for good." },
];

class MixItUp implements GameInstance {
  ...
  private buildPanel(): void {
    ...
    this.ctx.panel.append(
      el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🥣 Is it a"), el("span", {}, this.current().name)),
      el("div", { class: "control-label", style: { marginTop: "8px" } }, "Mixture, or a new substance?"),
      choices,
  ...
  private choose(saysMixture: boolean): void {
    if (this.ended) return;
    const it = this.current();
    if (saysMixture === it.mixture) { this.flash = 1; this.ctx.services.audio.play("tick"); this.coachEl.textContent = `✅ ${it.why}`; }
  ...
  private render(): void {
    ...
    if (this.ended) { c.fillStyle = "#831843"; c.font = "bold 30px Nunito, sans-serif"; c.textAlign = "center"; c.fillText("All sorted! 🥣", W / 2, H / 2); return; }
    ...
    c.fillStyle = "#fff"; c.strokeStyle = "#f9a8d4"; c.lineWidth = 6;
    ...
    c.fillStyle = "#831843"; c.font = "bold 24px Nunito, sans-serif"; c.fillText(it.name, cx, cy + 100);
    c.fillStyle = "#831843"; c.font = "bold 20px Nunito, sans-serif"; c.fillText("Can you separate it back out? 🥣", W / 2, 60);
  }
...
export const mixItUpGame: GameModule = {
  meta: {
    id: "mixitup", conceptId: "chem-10", title: "Mix It Up", stream: "chemistry", gradeBand: "5",
    emoji: "🥣", blurb: "Decide which things are mixtures you can separate and which are brand-new substances.",
    mission: "Sort each one into mixture or new substance.", estMinutes: 2,
  },
