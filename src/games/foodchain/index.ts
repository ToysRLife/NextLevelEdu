...
class FoodChain implements GameInstance {
  ...
  private pick(key: string): void {
    ...
    if (key === correct.key) {
      ...
      if (this.placed >= CHAIN.length) this.win();
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Chain complete! Energy flows from the Sun to plants and up through the animals that eat them. Remove any link and everything above it would go hungry.",
      stars,
  ...
  private render(): void {
    ...
    for (let i = 0; i < CHAIN.length; i++) {
      ...
      let col = i % cols;
      if (row % 2 === 1) col = cols - 1 - col; // snake layout
      const x = 160 + col * 240;
      const y = 200 + row * 200;
...
export const foodChainGame: GameModule = {
  meta: {
    id: "foodchain",
    conceptId: "bio-13",
    title: "Who Eats Whom",
