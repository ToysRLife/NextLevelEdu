...
class BasicNeeds implements GameInstance {
  ...
  private buildPanel(): void {
    ...
    const wantBtns = WANTS.map((w) =>
      el(
        "button",
        { class: "chip", onclick: () => this.give(w.key, false, w.label) },
        `${w.emoji} ${w.label}`
      )
    );
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Happy and healthy! Every living thing needs food, water, air, and shelter to survive. Toys and treats are just nice extras.",
      stars,
      resources: { Water: 40 },
    });
  }
  ...
  private render(): void {
    ...
    const cx = W / 2,
      cy = 300,
      bob = Math.sin(this.anim) * (4 + happy * 6);
    c.fillStyle = "#fbbf24";
    c.beginPath();
    ...
    c.arc(
      ...
      cy + 18 + bob,
      26,
      0.1 * Math.PI + (1 - happy) * 0.8,
      0.9 * Math.PI - (1 - happy) * 0.8
    );
    c.stroke();
    ...
    c.textAlign = "center";
    c.fillText(
      this.ended ? "Healthy and happy! ❤️" : "Give the creature its basic needs ❤️",
      W / 2,
      50
    );
  }
