...
class SortingCenter implements GameInstance {
  ...
  constructor(private readonly ctx: GameContext) {
    ...
    this.detach = onPointer(ctx.canvas, W, H, {
      down: (p) => { if (Math.hypot(p.x - this.tok.x, p.y - this.tok.y) < 46) this.dragging = true; },
      move: (p) => { if (this.dragging) { this.tok.x = p.x; this.tok.y = p.y; } },
      up: (p) => { if (this.dragging) { this.dragging = false; this.drop(p); } },
    });
    this.buildPanel();
  ...
  private buildPanel(): void {
    ...
    this.ctx.panel.append(
      el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Sort by"), el("span", {}, "Made of metal?")),
      el("div", { class: "control-label", style: { marginTop: "8px" } }, "Drag into the matching bin"),
  ...
  private drop(p: Point): void {
    ...
    if (chosen === null) { this.tok = { x: START.x, y: START.y }; return; }
    if (chosen === it.metal) { this.flash = 1; this.ctx.services.audio.play("tick"); this.coachEl.textContent = `✅ ${it.name} is ${it.metal ? "metal" : "not metal"}.`; }
    else { this.flash = -1; this.mistakes += 1; this.ctx.services.audio.play("fail"); this.coachEl.textContent = `❌ ${it.name} is ${it.metal ? "metal" : "not metal"}.`; }
    this.idx += 1;
  ...
  private render(): void {
    ...
    if (this.ended) { c.fillStyle = "#9a3412"; c.font = "bold 30px Nunito, sans-serif"; c.textAlign = "center"; c.fillText("All sorted! 📦", W / 2, H / 2); return; }

    const bin = (b: { x: number; y: number; w: number; h: number }, label: string, color: string) => {
      c.fillStyle = color; c.globalAlpha = 0.85; this.roundRect(c, b.x, b.y, b.w, b.h, 16); c.fill(); c.globalAlpha = 1;
      c.fillStyle = "#fff"; c.font = "bold 22px Nunito, sans-serif"; c.textAlign = "center"; c.fillText(label, b.x + b.w / 2, b.y + b.h / 2 + 8);
    };
    bin(BIN_METAL, "🔩 Metal", "#64748b");
    bin(BIN_NOT, "🚫 Not metal", "#a78bfa");

    const it = this.current();
    c.fillStyle = "#fff"; c.strokeStyle = this.dragging ? "#22c55e" : "#fdba74"; c.lineWidth = 4;
    c.beginPath(); c.arc(this.tok.x, this.tok.y, 42, 0, Math.PI * 2); c.fill(); c.stroke();
    c.font = "46px serif"; c.textAlign = "center"; c.fillText(it.emoji, this.tok.x, this.tok.y + 16);
    if (!this.dragging) { c.fillStyle = "#9a3412"; c.font = "bold 16px Nunito, sans-serif"; c.fillText(`Drag the ${it.name}`, this.tok.x, this.tok.y + 66); }
  ...
  pause(): void {}
  resume(): void {}
  reset(): void { this.ended = false; this.idx = 0; this.mistakes = 0; this.flash = 0; this.tok = { x: START.x, y: START.y }; this.dragging = false; this.order = [...ITEMS].sort(() => Math.random() - 0.5); this.ctx.services.hints.reset(); this.buildPanel(); }
  destroy(): void { cancelAnimationFrame(this.raf); this.detach(); }
}

export const sortingCenterGame: GameModule = {
  meta: {
    id: "sortingcenter", conceptId: "chem-16", title: "Sorting Center", stream: "chemistry", gradeBand: "K-2",
    emoji: "📦", blurb: "Drag objects into bins by a shared property — metal or not metal.",
    mission: "Sort every object by what it's made of.", estMinutes: 2,
  },
