...
class DayNight {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "cityAngle", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: Math.PI
        }); // city's position around Earth (0 = toward Sun)
        ...
        Object.defineProperty(this, "promptEl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    timeName() {
        ...
        if (cos < -0.92)
            return "🌙 Midnight";
        if (Math.abs(cos) < 0.16)
            return sin < 0 ? "🌅 Sunrise" : "🌇 Sunset";
        if (cos > 0)
            return sin < 0 ? "🌤️ Morning" : "🌤️ Afternoon";
        return sin < 0 ? "🌃 Late night" : "🌆 Evening";
    }
    buildPanel() {
        const lockBtn = el("button", { class: "btn", style: { background: "var(--accent-blue)" }, onclick: () => this.lockIn() }, "🔒 Lock the time");
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Make it"), this.promptEl), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Drag the city to spin Earth"), lockBtn, el("div", { class: "metric" }, el("span", {}, "🏙️ City time"), this.timeEl), el("div", { class: "metric" }, el("span", {}, "✅ Matched"), this.statusEl), this.coachEl);
        this.updateReadout();
    }
    updateReadout() {
    ...
    lockIn() {
        ...
        if (t.test(cos, sin)) {
            ...
            if (this.matched >= PROMPTS.length)
                ...
                this.promptEl.textContent = `${this.target().emoji} ${this.target().name}`;
        }
        else {
            this.mistakes += 1;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "You spun the day! Earth turns once a day on its axis. Whatever faces the Sun has daytime; the side turned away has night.",
            stars,
