...
class Ecosystem implements GameInstance {
  ...
  private renderLoop(): void {
    const draw = () => {
      ...
      if (!this.ended) {
        if (this.balanced()) {
          this.hold += 1;
          if (this.hold > byTier(this.ctx.tier, 34, 48, 64)) this.win();
        } else {
          this.hold = 0;
        }
      }
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "A thriving ecosystem! Living things depend on each other in a pyramid — many plants feed fewer plant-eaters, which feed even fewer predators. Balance keeps it healthy.",
      stars,
...
export const ecosystemGame: GameModule = {
  meta: {
    id: "ecosystem",
    conceptId: "bio-25",
