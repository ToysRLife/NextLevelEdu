...
function worldFor(stream) {
    return WORLDS.find((w) => w.stream === stream);
}
function canAfford(stream, cost) {
...
export function getWorldReward(stream) {
...
export function renderWorlds(root) {
    ...
    for (const w of WORLDS) {
        ...
        }, el("div", { class: "world-emoji" }, w.emoji), el("h3", {}, w.name), el("div", { class: "world-tagline" }, w.tagline), el("div", { class: "world-scene" }, ...(builtScene.length
            ? builtScene
            : [el("span", { class: "scene-empty" }, "Nothing built yet — go earn resources!")])), el("div", { class: "card-meter" }, el("div", { class: "meter-track" }, el("div", { class: "meter-fill", style: { width: `${pct}%` } }))), el("div", { class: "world-progress" }, el("span", { class: "tag" }, next ? `⏭️ Next: ${next.emoji} ${next.name}` : "🎉 Complete!"), el("span", { class: "tag" }, `📦 ${totalEarned}`))));
    }
    clear(root);
    root.append(el("div", { class: "container" }, el("h2", { class: "section-title" }, "Your Reward Worlds"), el("p", { class: "section-sub" }, "Every concept you master mints resources. Spend them here to build a world of your own."), grid));
}
// --- Single world: spend resources to raise structures, in order. ---
export function renderWorld(root, stream) {
    const w = worldFor(stream);
    if (!w) {
    ...
    for (const s of w.structures) {
        if (built.includes(s.id)) {
            stageRow.append(el("div", { class: "stage-structure", title: s.name }, el("div", { class: "st-emoji" }, s.emoji), el("div", { class: "st-label" }, s.name)));
        }
        else if (s === next) {
            stageRow.append(el("div", { class: "stage-structure ghost", title: `Next: ${s.name}` }, el("div", { class: "st-emoji" }, s.emoji), el("div", { class: "st-label" }, "next")));
            break; // don't reveal locked structures past the next one
        }
    }
    const stage = el("div", { class: `world-stage ${complete ? "complete" : ""}` }, el("div", { class: "stage-sky" }), el("div", { class: "stage-ground" }), stageRow, complete ? el("div", { class: "stage-banner" }, "🎉 World complete!") : null);
    // Completion meter — the big-picture progress toward finishing the world.
    const pct = Math.round((builtCount / total) * 100);
    const meter = el("div", { class: "world-meter" }, el("div", { class: "meter-track" }, el("div", { class: "meter-fill", style: { width: `${pct}%` } })), el("div", { class: "meter-label" }, `${builtCount} / ${total} built · ${pct}%`));
    ...
    if (next) {
        ...
        const resRows = Object.entries(next.cost).map(([name, amt]) => {
            ...
            return el("div", { class: "res-row" }, el("span", { class: "res-name" }, name), el("div", { class: "res-bar" }, el("div", { class: `res-fill ${cur >= amt ? "full" : ""}`, style: { width: `${p}%` } })), el("span", { class: "res-num" }, `${cur}/${amt}`));
        });
        ...
        focus = el("div", { class: `next-build ${affordable ? "ready" : ""}` }, el("div", { class: "nb-emoji" }, next.emoji), el("div", { class: "nb-info" }, el("div", { class: "nb-title" }, affordable ? `Ready to build: ${next.name}!` : `Next up: ${next.name}`), el("div", { class: "nb-blurb" }, next.blurb), ...resRows, action));
    }
    // Resource balances (all four shown, so goals are visible).
    const resourceBar = el("div", { class: "resource-bar" }, ...w.resources.map((name) => el("div", { class: "resource-chip" }, el("strong", {}, String(have[name] ?? 0)), el("span", {}, name))));
    ...
    for (const s of w.structures) {
        ...
        list.append(el("div", { class: `build-card ${stateClass}` }, el("div", { class: "build-emoji" }, s.emoji), el("div", { class: "build-info" }, el("h4", {}, s.name), el("div", { class: "build-blurb" }, s.blurb), el("div", { class: "build-cost" }, costLabel(s.cost))), action));
        prevBuilt = isBuilt;
    }
    clear(root);
    root.append(el("div", { class: `container world-view ${w.stream}` }, el("div", { class: "world-header" }, el("button", { class: "btn secondary", onclick: () => (location.hash = "#/worlds") }, "← Worlds"), el("div", { class: "world-title" }, el("span", { class: "world-emoji" }, w.emoji), el("h2", {}, w.name))), el("p", { class: "section-sub" }, w.tagline), stage, meter, focus, resourceBar, el("h3", { class: "build-roadmap-title" }, "🏗️ Build roadmap"), list));
}
