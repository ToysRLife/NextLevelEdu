...
class Resources {
    ...
    buildPanel() {
        const chips = el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...USES.map((u) => el("button", { class: "chip", onclick: () => this.choose(u.key) }, `${u.emoji} ${u.label}`)));
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🌍 Resource"), el("span", {}, this.current().name)), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Match it to its use"), chips, el("div", { class: "metric" }, el("span", {}, "♻️ Resource"), this.progressEl), this.coachEl);
    }
    choose(key) {
        if (this.ended)
            return;
        ...
        if (this.idx >= this.order.length) {
            ...
            this.progressEl.textContent = `${this.idx + 1} / ${RESOURCES.length}`;
    }
    win() {
        this.ended = true;
