...
class Tectonic {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "sizeRead", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    buildPanel() {
        this.chips = el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...BOUNDARIES.map((b) => el("button", {
        ...
        this.goBtn = el("button", { class: "btn", style: { background: "var(--accent-orange)" }, onclick: () => this.run() }, "🌍 Move the plates");
        clear(this.ctx.panel);
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Build"), el("span", {}, `${this.round().landform} (${this.round().target} m)`)), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Boundary type"), this.chips, this.speedCtl.el, this.sizeRead.el, this.goBtn, el("div", { class: "metric" }, el("span", {}, "✅ Landforms built"), this.statusEl), this.coachEl);
        this.updateReadout();
    }
    updateReadout() {
    ...
    finish() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Plate master! Convergent boundaries push crust up into mountains or down into trenches; divergent boundaries pull it apart into ridges and valleys. Faster plates build bigger features.",
            stars,
            resources: { Rock: 60 },
        });
    }
