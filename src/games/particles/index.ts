...
class Particles implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private readonly loop: SimLoop;

  private temp = 0.1; // 0..1
  private parts: P[] = [];
  ...
  private state(): State {
    if (this.temp < 0.34) return "solid";
    if (this.temp < 0.7) return "liquid";
    return "gas";
  }
  ...
  private tick(dtMs: number): void {
    ...
    if (s === this.target().state) {
      this.hold += f;
      if (this.hold > byTier(this.ctx.tier, 28, 40, 55)) this.matchOne();
    } else {
      this.hold = 0;
    }
    this.render();
  ...
  private finish(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "You see it now! Matter is made of tiny particles. Add heat and they move more — locked solid, flowing liquid, then free-flying gas.",
      stars,
...
export const particlesGame: GameModule = {
  meta: {
    id: "particles",
    conceptId: "chem-19",
    title: "Inside Matter",
