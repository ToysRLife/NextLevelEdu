...
class Crystals implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private readonly loop: SimLoop;
  private readonly slow: number; // cooling speed below this stays clear (adaptive)

  private speed = 0.25;
  private size = 0; // 0..1
  private clarity = 1; // 0..1
  private anim = 0;
  private ended = false;
  ...
  private tick(dtMs: number): void {
    ...
    if (this.size >= 1 && this.clarity >= 0.75) this.finish();
    this.render();
  }
  ...
  private finish(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "A beautiful crystal! Cooling slowly let the particles line up into one big, clear crystal with flat faces. Rushing makes a cloudy clump of tiny ones.",
      stars,
...
export const crystalsGame: GameModule = {
  meta: {
    id: "crystals",
    conceptId: "chem-17",
