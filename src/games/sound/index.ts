...
class Sound implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private readonly loop: SimLoop;

  private tension = 50; // 0..100 -> pitch
  private volume = 60; // 0..100 -> amplitude
  private phase = 0;
  private order: typeof NOTES = [];
  private idx = 0;
  private matched = 0;
  private mistakes = 0;
  private ended = false;
  ...
  private target(): (typeof NOTES)[number] {
  ...
  private buildPanel(): void {
    const tensionSlider = el("input", {
      ...
      oninput: (e: Event) => {
        this.tension = Number((e.target as HTMLInputElement).value);
        this.updateReadout();
      },
    });
  ...
  private finish(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Perfectly tuned! Sound is vibration. Faster vibrations make a higher pitch, slower ones a lower pitch — and volume just changes how big the vibration is.",
      stars,
      resources: { Fuel: 40 },
    });
  }
...
export const soundGame: GameModule = {
  meta: {
    id: "sound",
    conceptId: "phys-17",
    title: "Pitch Perfect",
