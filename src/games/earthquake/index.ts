...
interface Round {
  magnitude: number; // shake strength
  needed: number; // stability score required
  label: string;
}
...
class Earthquake implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private raf = 0;
  private t = 0;
  private ended = false;

  private wideBase = false;
  private braced = false;
  private lightTop = false;

  private roundIdx = 0;
  private shaking = false;
  private shakeT = 0;
  private collapsed = false;
  private collapseBlocks: Block[] = [];
  private attempts = 0;
  ...
  private shake(): void {
    if (this.ended || this.shaking || this.collapsed) return;
    this.shaking = true;
    this.shakeT = 0;
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Quake-proof! A wide base, cross-bracing, and keeping weight low help a building stand through an earthquake's shaking.",
      stars,
      resources: { Rock: 40 },
    });
  }
  ...
  private renderLoop(): void {
    const draw = () => {
      ...
      if (this.shaking) {
        this.shakeT += 1;
        if (this.shakeT > 90) this.resolve();
      }
      if (this.collapsed) {
        let settled = true;
...
export const earthquakeGame: GameModule = {
  meta: {
    id: "earthquake",
    conceptId: "ess-18",
    title: "Shake-Proof",
