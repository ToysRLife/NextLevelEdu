...
class HotCold implements GameInstance {
  ...
  private buildPanel(): void {
    const chips = el(
      ...
      ...RESULTS.map((r) =>
        el("button", { class: "chip", onclick: () => this.choose(r.key) }, `${r.emoji} ${r.label}`)
      )
    );
    this.progressEl = el(
      "span",
    ...
    this.ctx.panel.append(
      el(
        ...
        el("span", {}, cur.action === "heat" ? "🔥 Heat it" : "🧊 Cool it"),
        el("span", {}, cur.item)
      ),
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Nicely predicted! Heating melts and boils things; cooling freezes them. Temperature changes the state of matter.",
      stars,
      resources: { Energy: 40 },
    });
  }
