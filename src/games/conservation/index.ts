...
class Conservation implements GameInstance {
  ...
  private tilt = 0; // animated scale tilt
  ...
  private guess(a: Answer): void {
    if (this.ended || this.revealed) return;
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "You've got it! Mass is conserved — matter never just vanishes. In a sealed container the mass always stays the same; it only seems to drop when a gas escapes.",
      stars,
...
export const conservationGame: GameModule = {
  meta: {
    id: "conservation",
    conceptId: "chem-18",
    title: "Nothing Lost",
