...
export function makeFirebaseProvider(config) {
    let ready = null;
    let authMod = null;
    let fsMod = null;
    let auth = null;
    ...
    function cacheApproval(uid, approved) {
        try {
    ...
    function maybeNotify(uid, email, name) {
        if (!SIGNUP_NOTIFY_URL)
            return;
        try {
    ...
    return {
        ...
        async approveUser(uid, email = "", name = "") {
            ...
            if (email)
                postNotify({ type: "approved", email, name, time: new Date().toISOString() });
        },
        async listFeedback() {
            await init();
        ...
        async accountStatus(user) {
            ...
            try {
                ...
                if (snap.exists()) {
                    ...
                    approved = hasSave;
                    await fsMod.setDoc(ref, hasSave
                        ? {
                            email,
                            name,
                            approved: true,
                ...
                if (!approved)
                    maybeNotify(user.uid, email, name);
                return { approved };
            }
            catch {
