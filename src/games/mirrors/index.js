...
class Mirrors {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "holdFrames", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        }); // beam must rest on target briefly
        ...
        Object.defineProperty(this, "emitDeg", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: -10
        }); // emission angle in degrees (0 = pointing right)
        Object.defineProperty(this, "mirrors", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: [
    ...
    buildPanel() {
        const angleSlider = el("input", {
            ...
            oninput: (e) => {
                this.emitDeg = Number(e.target.value);
            },
        });
        const rot1 = el("button", { class: "btn secondary", onclick: () => this.rotate(0) }, "🔄 Rotate Mirror 1");
        const rot2 = el("button", { class: "btn secondary", onclick: () => this.rotate(1) }, "🔄 Rotate Mirror 2");
        this.coachEl = el("div", {
            class: "hint-panel",
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Goal"), el("span", {}, "Light up the target")), el("div", { class: "control-label", style: { marginTop: "8px" } }, "🔦 Aim the light"), angleSlider, el("div", { class: "control-label" }, "Rotate the mirrors"), rot1, rot2, this.coachEl);
    }
    rotate(i) {
        if (this.ended)
            return;
    ...
    trace() {
        ...
        let d = { x: Math.cos(rad), y: Math.sin(rad) };
        let hit = false;
        for (let bounce = 0; bounce < 8; bounce++) {
            ...
            for (const m of this.mirrors) {
                ...
                if (hitInfo && (!best || hitInfo.t < best.t)) {
                    const n = { x: -Math.sin(m.angle), y: Math.cos(m.angle) };
    ...
    segmentHitsCircle(a, b, circ) {
        const abx = b.x - a.x;
        const aby = b.y - a.y;
        const len2 = abx * abx + aby * aby;
        if (len2 < 1e-6)
            return Math.hypot(a.x - circ.x, a.y - circ.y) <= circ.r;
    ...
    render() {
        ...
        if (hit && !this.ended) {
            this.holdFrames += 1;
            if (this.holdFrames > byTier(this.ctx.tier, 10, 18, 28))
                this.win();
        }
        else if (!hit) {
            this.holdFrames = 0;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Target lit! Light travels in straight lines and bounces off mirrors so that the angle in equals the angle out — that's the law of reflection.",
            stars,
            resources: { Power: 40 },
        });
    }
