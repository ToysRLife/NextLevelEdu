...
class Digestion implements GameInstance {
  ...
  private pick(key: string): void {
    ...
    if (key === correct.key) {
      ...
      if (this.placed >= STEPS.length) this.win();
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Journey complete! Food is chewed in the mouth, mashed in the stomach, absorbed in the small intestine, and the leftovers pass through the large intestine.",
      stars,
...
export const digestionGame: GameModule = {
  meta: {
    id: "digestion",
    conceptId: "bio-23",
    title: "Food Journey",
