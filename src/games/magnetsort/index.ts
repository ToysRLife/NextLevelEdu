...
class MagnetSort implements GameInstance {
  ...
  private buildPanel(): void {
    const choices = el(
      ...
      el(
        "button",
        { class: "btn secondary", onclick: () => this.choose(false) },
        "🚫 Doesn't stick"
      )
    );
    this.progressEl = el(
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Sorted! Magnets only stick to iron, steel, and a few metals like nickel — not plastic, wood, rubber, or most other metals.",
      stars,
      resources: { Alloy: 40 },
    });
  }
