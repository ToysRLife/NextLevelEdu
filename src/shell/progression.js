...
export function hasWon(id) {
    return read(`progress:${id}`, { wins: 0 }).wins > 0;
}
export function streamGames(stream) {
...
export function currentLevel(stream) {
...
export function streamProgress(stream) {
    const games = streamGames(stream);
...
export function nextUpFor(stream) {
    const games = streamGames(stream);
    const lvl = currentLevel(stream);
