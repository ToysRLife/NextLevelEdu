...
class Precipitation {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "moisture", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0.3
        ...
        Object.defineProperty(this, "nowEl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    precip() {
        if (this.moisture < 0.5)
            return "none";
        if (this.temp < 0.35)
            return "snow";
        if (this.moisture > 0.85)
            return "hail";
        return "rain";
    }
    buildPanel() {
        ...
        const moistSlider = el("input", {
            ...
            oninput: (e) => {
                this.moisture = Number(e.target.value) / 100;
                this.updateReadout();
            },
        });
        this.nowEl = el("span", {}, "");
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Forecast"), el("span", {}, `${this.target().emoji} ${this.target().label}`)), el("div", { class: "control-label", style: { marginTop: "8px" } }, "🌡️ Temperature (cold → hot)"), tempSlider, el("div", { class: "control-label" }, "💧 Moisture (dry → humid)"), moistSlider, el("div", { class: "metric" }, el("span", {}, "☁️ Now falling"), this.nowEl), el("div", { class: "metric" }, el("span", {}, "✅ Matched"), this.statusEl), this.coachEl);
        this.updateReadout();
    }
    label(p) {
        return p === "rain"
    ...
    tick(dtMs) {
        ...
        for (const d of this.drops) {
            d.y += d.v * f;
            if (d.kind === "snow")
                d.x += Math.sin(d.y * 0.05) * 0.6 * f;
        }
        this.drops = this.drops.filter((d) => d.y < 520 && d.kind === p);
        if (p === this.target().kind) {
            this.hold += f;
            if (this.hold > byTier(this.ctx.tier, 26, 36, 50))
                this.matchOne();
        }
        else {
            this.hold = 0;
    ...
    finish() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Weather wizard! When clouds hold enough moisture it precipitates — warm air makes rain, freezing air makes snow, and big storms make hail.",
            stars,
            resources: { Water: 40 },
        });
    }
...
export const precipitationGame = {
    meta: {
        id: "precipitation",
        conceptId: "ess-11",
        title: "Weather Maker",
