...
const BATTERY: Point = { x: LX, y: (TY + BY) / 2 };
const BULB: Point = { x: (LX + RX) / 2, y: TY };

interface Toggle {
  id: "wireA" | "wireB" | "short";
  x: number;
  y: number;
  label: string;
  closed: boolean;
...
class Circuits implements GameInstance {
  ...
  private evaluate(): void {
    ...
    if (short) {
      if (bulbState) bulbState.textContent = "SHORT!";
      this.fail();
      return;
    }
    if (this.lit()) {
      if (bulbState) bulbState.textContent = "On 💡";
      this.win();
    } else {
      if (bulbState) bulbState.textContent = "Off";
  ...
  private seg(c: CanvasRenderingContext2D, a: Point, b: Point, on: boolean): void {
  ...
  private render(): void {
    ...
    if (lit) {
      const path: Point[] = [TL, BULB, TR, BR, BL, TL];
  ...
  reset(): void {
    this.ended = false;
    this.flow = 0;
    for (const t of this.toggles) t.closed = false;
...
export const circuitsGame: GameModule = {
  meta: {
    id: "circuits",
    conceptId: "phys-21",
    title: "Light It Up",
