...
class BalloonPop implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private readonly scoreEl: HTMLElement;
  private readonly hitsEl: HTMLElement;
  private readonly missesEl: HTMLElement;
  ...
  private readonly opButtons: HTMLElement[] = [];
  ...
  private balloons: BalloonState[] = [];
  private particles: Array<ConfettiParticle | SadParticle> = [];
  ...
  constructor(private readonly ctx: GameContext) {
    ...
    this.resetBtn = el(
      "button",
      { class: "btn secondary", onclick: () => this.resetGame() },
      "Reset"
    );
  ...
  private attachCanvas(): void {
    const onClick = (event: MouseEvent) => {
      if (!this.problemActive || !this.running) return;
  ...
  private buildPanel(): void {
    const operationRow = el(
      "div",
      { class: "chip-row", style: { gap: "10px", marginTop: "12px", marginBottom: "12px" } },
      ...OPERATIONS.map((op) => {
        const button = el(
          "button",
          {
    ...
    this.ctx.panel.append(
      el(
        ...
        el("div", { style: { fontSize: "1.2rem", fontWeight: 900 } }, "🎈 Balloon Pop"),
        el(
          "div",
          { class: "metric", style: { flex: "0 0 auto", padding: "10px 14px", margin: 0 } },
          el("span", {}, "Score"),
          this.scoreEl
        )
      ),
      this.problemEl,
      this.instructionEl,
      el("div", { class: "control-label" }, "Operation"),
      operationRow,
      el("div", { class: "metric" }, el("span", {}, "✅ Correct"), this.hitsEl),
      el("div", { class: "metric" }, el("span", {}, "❌ Wrong"), this.missesEl),
      el(
        "div",
  ...
  private sampleProblem(): string {
    if (this.currentOperation === "+") return "8 + 3";
    if (this.currentOperation === "-") return "12 - 4";
    if (this.currentOperation === "*") return "5 × 6";
    return "24 ÷ 6";
  }
  ...
  private renderProblemCard(problem: string, findValue: number): void {
    clear(this.problemEl);
    this.problemEl.append(
      el("div", { style: { fontWeight: 900, marginBottom: "8px" } }, problem),
      el(
        "div",
        { style: { fontWeight: 400, fontSize: "0.95rem", color: "var(--text-muted)" } },
        `Find: ${findValue}`
      )
    );
  }
  ...
  private handleBalloonClick(balloon: BalloonState): void {
  ...
  private spawnParticles(x: number, y: number, success: boolean): void {
  ...
  private drawBalloon(c: CanvasRenderingContext2D, balloon: BalloonState): void {
    if (balloon.popping) {
  ...
  private updateBalloon(balloon: BalloonState, index: number): void {
    if (balloon.popping) {
      balloon.popTime += 0.016;
      if (balloon.popTime >= 0.3) this.balloons.splice(index, 1);
      return;
    }

    balloon.y -= balloon.speed;
    balloon.x += balloon.vx;
    if (balloon.x < -BALLOON_RADIUS) balloon.x = W + BALLOON_RADIUS;
    if (balloon.x > W + BALLOON_RADIUS) balloon.x = -BALLOON_RADIUS;
...
export const balloonGame: GameModule = {
  meta: {
    id: "balloon",
    conceptId: "phys-02",
    title: "Balloon Pop",
