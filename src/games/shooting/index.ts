...
class Shooting implements GameInstance {
  ...
  private misses = 0;
  private rounds = 0;
  private actionKeys: Record<string, boolean> = {};
  ...
  private readonly opButtons: HTMLButtonElement[] = [];
  ...
  private attachKeyboard(): void {
    const down = (e: KeyboardEvent) => {
      if (e.key === "ArrowUp") {
        e.preventDefault();
        if (this.running && this.problemActive) this.fireBullet();
      }
  ...
  private buildPanel(): void {
    ...
    const operationRow = el(
      "div",
      { class: "chip-row", style: { flexWrap: "wrap", gap: "8px", marginBottom: "12px" } },
      ...OPERATIONS.map((op) => {
        const button = el(
          "button",
          {
  ...
  private updateProblemPreview(): void {
    const sample = this.sampleProblem();
    this.problemEl.textContent = sample.text;
  }

  private sampleProblem(): { text: string } {
    if (this.currentOperation === "+") return { text: "8 + 3" };
    if (this.currentOperation === "-") return { text: "12 - 5" };
    if (this.currentOperation === "*") return { text: "7 × 6" };
    return { text: "24 ÷ 6" };
  }
  ...
  private makeProblem(): { a: number; b: number; answer: number; text: string } {
    let a: number;
    let b: number;
    let answer: number;
    let text: string;
  ...
  private makeWrongAnswer(correct: number): number {
    ...
    if (wrong <= 0) wrong = correct + delta + 1;
    return wrong;
  }
  ...
  private updateGame(): void {
    if (!this.running || !this.problemActive) return;
    ...
    for (let i = this.bullets.length - 1; i >= 0; i -= 1) {
      ...
      if (bullet.y + bullet.height < 0) this.bullets.splice(i, 1);
    }
    ...
    for (let i = this.airplanes.length - 1; i >= 0; i -= 1) {
      ...
      plane.y += 1;
      plane.x += plane.vx;
      if (plane.x < 20) plane.x = 20;
      if (plane.x > W - plane.width - 20) plane.x = W - plane.width - 20;
  ...
  private handleHit(plane: Airplane): void {
    if (!plane.isCorrect) {
      this.misses += 1;
  ...
  private handleEscape(plane: Airplane): void {
    if (plane.isCorrect) {
      this.misses += 1;
      this.feedbackEl.className = "feedback incorrect";
  ...
  private drawGun(c: CanvasRenderingContext2D): void {
    c.save();
    c.fillStyle = "#111827";
...
export const shootingGame: GameModule = {
  meta: {
    id: "shooting",
    conceptId: "phys-02",
