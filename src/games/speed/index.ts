...
class Speed implements GameInstance {
  ...
  private go(): void {
    if (this.ended || this.driving) return;
    this.driving = true;
    this.progress = 0;
    this.elapsed = 0;
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Delivered on time! Speed is distance divided by time — go the right speed and you cover the distance in exactly the time you need.",
      stars,
      resources: { Power: 40 },
    });
  }
...
export const speedGame: GameModule = {
  meta: {
    id: "speed",
    conceptId: "phys-02",
