...
export interface LineGraphOpts {
  points: [number, number][]; // data in graph units
  xMax: number;
  yMax: number;
  color?: string;
  title?: string;
  xLabel?: string;
  yLabel?: string;
  /** Optional horizontal target band (in y units) to shade, e.g. a goal speed. */
...
export function drawLineGraph(
  ...
  const sx = (v: number) => gx + (v / o.xMax) * gw;
  const sy = (v: number) => gy + gh - (v / o.yMax) * gh;
  ...
  c.textAlign = "center";
  if (o.xLabel) c.fillText(o.xLabel, gx + gw / 2, y + h - 4);
  if (o.yLabel) {
    c.save();
