...
class Irreversible {
    ...
    buildPanel() {
        ...
        const choiceRow = el("div", { class: "chip-row" }, el("button", {
            class: "btn",
            style: { background: "var(--accent-blue)" },
            onclick: () => this.choose(true),
        }, "♻️ Reversible"), el("button", {
            class: "btn",
            style: { background: "var(--accent-red)" },
            onclick: () => this.choose(false),
        }, "🔥 Irreversible"));
        clear(this.ctx.panel);
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Goal"), el("span", {}, "Sort all 8 changes")), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Is this change reversible?"), choiceRow, el("div", { class: "metric" }, el("span", {}, "🧩 Item"), this.progressEl), this.coachEl);
    }
    choose(saysReversible) {
        if (this.ended)
            return;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Sorted! Reversible changes keep the same substance (melting, boiling). Irreversible changes make brand-new stuff (burning, cooking, rusting).",
            stars,
            resources: { Energy: 40 },
        });
    }
