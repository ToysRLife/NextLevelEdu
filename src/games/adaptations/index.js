...
class Adaptations {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "survived", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    buildPanel() {
        const rows = CATEGORIES.map((cat) => el("div", {}, el("div", { class: "control-label" }, cat.label), el("div", { class: "chip-row", style: { flexWrap: "wrap" } }, ...cat.options.map((o) => {
        ...
        const releaseBtn = el("button", { class: "btn", style: { background: "var(--accent-green)" }, onclick: () => this.release() }, "🌿 Release into the wild");
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Survive"), el("span", {}, this.habitat().name)), ...rows, releaseBtn, el("div", { class: "metric" }, el("span", {}, "🏆 Survived"), this.statusEl), this.coachEl);
    }
    release() {
        if (this.ended)
            return;
        ...
        const wrong = Object.keys(h.correct).filter((k) => picks[k] !== h.correct[k]);
        if (wrong.length === 0) {
            this.survived += 1;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "Survivors all! Adaptations are body features matched to a habitat — fur and blubber for cold, water stores for deserts, claws and grips for the forest.",
            stars,
    ...
    render() {
        ...
        for (const cat of CATEGORIES) {
            ...
            if (sel)
                chosen.push(cat.options.find((o) => o.key === sel).emoji);
        }
        c.font = "34px serif";
        chosen.forEach((e, i) => {
