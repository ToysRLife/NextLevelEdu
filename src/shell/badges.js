...
const STREAMS = ["physics", "chemistry", "biology", "earth-space"];
export const BADGES = [
    {
        id: "first-steps",
        emoji: "🐣",
...
export function gatherStats() {
    ...
    for (const m of GAME_MANIFESTS) {
        const p = read(`progress:${m.id}`, { wins: 0, bestStars: 0, attempts: 0 });
        if (p.attempts > 0)
            played += 1;
        if (p.wins > 0) {
            wins += 1;
    ...
    for (const stream of STREAMS) {
        ...
        if (getWorldReward(stream)?.complete)
            worldsComplete += 1;
    }
    return {
        played,
...
export function checkBadges() {
    const earned = new Set(read("badges:earned", []));
