...
function mountNav(root, view) {
    ...
    const nav = el("nav", {}, el("div", { class: "nav-brand", onclick: () => (location.hash = "#/") }, "NextLevel ", el("span", {}, "Edu")), el("div", { class: "nav-links" }, el("button", { class: "nav-link", onclick: () => (location.hash = "#/") }, "🎯 Missions"), el("button", { class: "nav-link", onclick: () => (location.hash = "#/worlds") }, "🏗️ Worlds"), el("button", { class: "nav-link", onclick: () => (location.hash = "#/badges") }, "🏅 Trophies"), cloud().isAdmin()
        ? el("button", { class: "nav-link", onclick: () => (location.hash = "#/admin") }, "🛠️ Admin")
        : null, cloudBtn(), muteBtn()), el("div", { class: "persona" }, streakEl, joulesEl, el("div", { class: "persona-avatar" }, persona.emoji), el("div", { class: "persona-meta" }, el("strong", {}, persona.alias), rankEl)));
    clear(root);
...
function refreshStats() {
    const joules = getJoules();
    if (joulesEl)
        joulesEl.textContent = `⚡ ${joules}`;
    if (rankEl)
        rankEl.textContent = rankForJoules(joules);
    if (streakEl)
        streakEl.textContent = `🔥 ${getStreak().current}`;
    if (cloudEl)
        cloudEl.textContent = cloudLabel();
}
