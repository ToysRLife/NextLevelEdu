...
class Inherited {
    ...
    randomise() {
        for (const t of TRAITS)
            this.source[t.key] = Math.random() < 0.5 ? "mom" : "dad";
    }
    current() {
    ...
    buildPanel() {
        ...
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🧬 Trait"), el("span", {}, `${this.current().emoji} ${this.current().label}`)), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Which parent gave this trait?"), choices, el("div", { class: "metric" }, el("span", {}, "🐱 Trait"), this.progressEl), this.coachEl);
    }
    choose(parent) {
        if (this.ended)
            return;
    ...
    win() {
        ...
        this.ctx.services.outcome.succeed({
            message: "It's all in the family! Offspring inherit their traits from their parents — that's why a baby animal looks like a mix of its mum and dad.",
            stars,
    ...
    drawCat(c, x, y, fur, ears, tail, label, highlight) {
        // body
        ...
        c.moveTo(x + 40, y + 20);
        if (tail === "long")
            c.quadraticCurveTo(x + 90, y - 10, x + 80, y - 60);
        else
            c.quadraticCurveTo(x + 60, y + 10, x + 64, y - 6);
        c.stroke();
        // face
        c.fillStyle = "#1e293b";
        c.beginPath();
    ...
    render() {
        ...
        this.drawCat(c, 400, 300, this.babyValue(TRAITS[0]), this.babyValue(TRAITS[1]), this.babyValue(TRAITS[2]), "👶 Baby", hl);
        c.fillStyle = "#581c87";
