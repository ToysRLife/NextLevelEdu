...
class Rainbow implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private raf = 0;
  private anim = 0;
  private ended = false;

  private aimDeg = 20; // exit angle of the rainbow's centre
  private targetY = 300;
  private caught = 0;
  private hold = 0;
  private readonly tol: number; // how close the rainbow must land (adaptive)
  ...
  private buildPanel(): void {
    const slider = el("input", {
      ...
      oninput: (e: Event) => {
        this.aimDeg = Number((e.target as HTMLInputElement).value);
        this.updateReadout();
      },
    });
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "Rainbows made! White light is a mix of colours. A prism bends each colour differently — red least, violet most — fanning them into a rainbow.",
      stars,
      resources: { Power: 40 },
    });
  }
...
export const rainbowGame: GameModule = {
  meta: {
    id: "rainbow",
    conceptId: "phys-15",
