...
class Separating {
    constructor(ctx) {
        ...
        Object.defineProperty(this, "leftEl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    buildPanel() {
        ...
        const toolRow = el("div", { class: "chip-row" }, ...METHODS.map((m) => el("button", {
            class: "chip",
            "data-method": m.id,
            onclick: () => this.applyMethod(m.id),
        }, `${m.emoji} ${m.label}`)));
        clear(this.ctx.panel);
        this.ctx.panel.append(el("div", { class: "metric", style: { background: "#1e293b", color: "#fff" } }, el("span", {}, "🎯 Goal"), el("span", {}, "Separate it all")), el("div", { class: "control-label", style: { marginTop: "8px" } }, "Choose a separation tool"), toolRow, el("div", { class: "metric" }, el("span", {}, "🧪 Parts left"), this.leftEl), this.coachEl);
    }
    applyMethod(method) {
        if (this.ended)
            return;
    ...
    reset() {
        this.ended = false;
        this.mistakes = 0;
        for (const cc of this.components)
            cc.separated = false;
