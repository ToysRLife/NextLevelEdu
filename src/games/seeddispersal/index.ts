...
class SeedDispersal implements GameInstance {
  private readonly ctx2d: CanvasRenderingContext2D;
  private readonly loop: SimLoop;

  private wind = 0; // current gust strength 0..1 (oscillating)
  private windDir = 1;
  private seeds: Seed[] = [];
  private launched = 0;
  private reached = 0;
  ...
  private tick(dtMs: number): void {
    ...
    for (const s of this.seeds) {
      ...
      if (s.y >= GROUND) {
        ...
        if (this.reached >= NEED) this.finish();
      }
    }
...
export const seedDispersalGame: GameModule = {
  meta: {
    id: "seeddispersal",
    conceptId: "bio-06",
    title: "Ride the Wind",
