...
interface Constellation {
  name: string;
  emoji: string;
  stars: { x: number; y: number }[]; // connected in array order
}
...
class Constellations implements GameInstance {
  ...
  private idx = 0;
  private connected = 0; // stars connected in current constellation
  private mistakes = 0;
  private bgStars: { x: number; y: number; s: number }[] = [];
  private doneLines: { a: { x: number; y: number }; b: { x: number; y: number } }[] = [];
  ...
  private win(): void {
    ...
    this.ctx.services.outcome.succeed({
      message:
        "The sky is mapped! Constellations are star patterns people connected long ago to tell stories and find directions at night.",
      stars,
  ...
  private render(): void {
    ...
    c.textAlign = "center";
    if (!this.ended) c.fillText(`Trace ${cons!.name} ${cons!.emoji}`, W / 2, 40);
  }
...
export const constellationsGame: GameModule = {
  meta: {
    id: "constellations",
    conceptId: "ess-06",
